...
public class ProceduralAbstraction {
    ...
    //Requires: None
    //Modifies: None
    //Effects: If the matrix array satisfies Nice property, prints the sum and
    // returns true. Otherwise returns false.
    public static boolean isMatrixNice(int[][] array) {
        int length = array.length; //Stores the length 

        //Check if matrix is a square
        for (int i = 0; i < length; i++) //Loop through all rows
        {
            if (array[i].length != length) //Check if lengths are equal
            {
        ...
        {
            sum += array[i][i]; //Calculate sum
        }
        //Calculate row and column sums
        for (int i = 0; i < length; i++) {
            int rowSum = 0; //Stores the sum of the row currently being checked
            int columnSum = 0; //Stores the sum of the column currently being checked

            for (int j = 0; j < length; j++) {
                rowSum += array[i][j]; //Calculate current row sum
                columnSum += array[j][i]; //Calculate current column sum
            }

            if (rowSum != sum || columnSum != sum) //Check if sums are equal
            {
        ...
        for (int i = 0; i < length; i++) {
            diag1Sum += array[i][i]; //Calculate the sum of the first diagonal
            diag2Sum += array[i][length - i - 1]; //Calculate the sum of the second diagonal
        }
